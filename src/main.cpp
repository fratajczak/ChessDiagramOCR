...
bool isOnBlackSquare(Mat & piece)
{
	...
	black_corners = (mean(piece(Rect(0,0,4,4)))[0] < 10) + (mean(piece(Rect(28,0,4,4)))[0] < 10)
...
void normalizePiece(Mat & piece)
{
	...
	copyMakeBorder(piece, piece, border_v, border_v, border_h, border_h, BORDER_CONSTANT, Scalar(255,255,255));
...
{{"--correct-fen=FEN", "classifies pieces according to a FEN position when using --output-folder"},
